...
const checkPolicy = (inputOp)=>{
  const {min,max,letter,pass} = inputOp;
  const re = new RegExp(letter, 'g');
  const found = pass.match(re) || [];
  const isValid = found.length <=max && found.length>=min;
  return isValid
}
const checkPolicyReal = (inputOp)=>{
  const {min,max,letter,pass} = inputOp;
  const isValid = (pass[min-1]==letter || pass[max-1]==letter) &&
...
// arr.forEach((a,i)=>{  
//  arr.forEach((b,i)=>{
//    const op = getSum(a,b,c);
//    if(op){
//      console.log("numbers are: "+a+" and "+b+"and "+ c +"product is "+op);
//    }
//  })
// })
// })

let dataStr= `14-15 v: vdvvvvvsvvvvvfpv
...
const finalOp = rows.map(r=> getPolicy(r));

const invalid = finalOp.reduce((a,b)=>{
  if(!checkPolicyReal(b))  a++;
  return a
},0)

console.log(rows.length-invalid);
